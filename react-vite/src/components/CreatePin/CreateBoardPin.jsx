...
function CreateBoardPin( props ) {
  ...
  function onImageChange(e) {
   ...
   async function onSubmit(e) {
     ...
     const response = await dispatch(thunkPostBoardPins(payload));
     if (response.errors) {
       setErrors({ errors: Object.values(response.errors) });
       setDisabled(false);
       return;
     } else {
       if (props.onPinCreated) {
       props.onPinCreated(response);
       props.onClose();
       }
     }
